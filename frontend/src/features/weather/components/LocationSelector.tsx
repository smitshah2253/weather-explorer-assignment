...
export function LocationSelector({
  ...
  return (
    <div className="glass-panel rounded-2xl p-3.5 sm:p-4 lg:p-5 flex flex-col justify-between gap-3 shadow-xs h-[320px] sm:h-[380px] lg:h-[460px] xl:h-[480px] w-full font-sans">
      {/* Header */}
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="bg-muted/60 px-2 py-0.5 rounded-md border border-border/60 text-[10px] sm:text-[11px] font-mono text-foreground tabular-nums whitespace-nowrap" aria-label={`Coordinates: ${latitude}, ${longitude}`}>
          ...
          <Button
            ...
            size="icon"
            onClick={onReset}
            className="h-8 w-8 sm:h-7 sm:w-7 text-muted-foreground hover:text-foreground rounded-md cursor-pointer shrink-0"
      ...
      {/* Map */}
      <div className="relative flex-1 min-h-[160px] sm:min-h-[200px] w-full rounded-xl overflow-hidden border border-border/60 bg-muted/20" role="application" aria-label="Interactive location map">
        <MapContainer
      ...
      {/* Presets */}
      <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none shrink-0 py-0.5 -mx-1 px-1" role="toolbar" aria-label="Quick city presets">
        {PRESET_LOCATIONS.map((preset: PresetLocation) => {
          ...
          return (
            <button
              ...
              className={`text-[11px] px-2.5 py-1.5 sm:px-2 sm:py-1 rounded-md font-medium transition-all shrink-0 cursor-pointer border focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1 min-h-[32px] sm:min-h-[26px] flex items-center ${
                isSelected
